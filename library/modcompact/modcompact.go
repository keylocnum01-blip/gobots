...
func TMoreShoot(decodedByte []byte) *TMoreCompactProtocol {
...
func (p *TMoreCompactProtocol) GetSync5Response() (a *sync4.SyncResponse, err *ExceptionMod) {
	jsonStr, _ := json.Marshal(p.res)
	if p.err {
		json.Unmarshal(jsonStr, &err)
	} else {
		json.Unmarshal(jsonStr, &a)
	}
	return a, err
}

func (p *TMoreCompactProtocol) GetChatsResponse() (res *linethrift.GetChatsResponse, err *ExceptionMod) {
	jsonStr, _ := json.Marshal(p.res)
	if p.err {
		json.Unmarshal(jsonStr, &err)
	} else {
...
func (p *TMoreCompactProtocol) GetAddFriend() (res map[string]*linethrift.Contact, err *ExceptionMod) {
  if p.err == true {
		jsonStr, _ := json.Marshal(p.res)
		json.Unmarshal(jsonStr, &err)
	} else {
...
func TMoreCompact() {
	...
	d := fmt.Sprintf("2024-10-15T00:00:00Z")
	has := strings.Split(d, "-")
